...
const fetchData = async () => {
  if (!props.marketId || pageable.last || isLoading.value) {
    return
  }
  isLoading.value = true

  const result = await new ProductService().service.getSolutions({
    marketId: props.marketId,
    contentLanguage: user.value.contentLanguage.id,
    pageable: {
      page: pageable.page,
      size: pageable.size,
    },
  ...
  <div class="d-flex flex-column">
    <div class="extend-items">
      ...
      >
        <div class="icon">
          <AuthImage
            v-if="item.crop.iconImageUrl"
            :height="169"
            :path="item.crop.iconImageUrl"
            :width="128"
          />
          <img
            v-else
