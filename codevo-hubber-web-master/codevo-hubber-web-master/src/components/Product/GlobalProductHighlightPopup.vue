...
onMounted(() => {
  data.value = {
    claim: clone(props.product?.claim || ''),
    highlights: clone(props.product?.highlights || []),
  }
})
...
const addContent = () => {
  if (data.value?.highlights?.length) {
    data.value.highlights = [...data.value.highlights, '']
  } else {
...
const deleteContent = (index) => {
  const tmp = clone(data.value.highlights)
  tmp.splice(index, 1)
  data.value.highlights = tmp
}

const confirmData = async () => {
  // Update product highlight
  await store.commit('products/updateCurrentProduct', data.value)
  onClose()
}
</script>
      ...
      <v-row no-gutters>
        <v-col sm="12">
          ...
          <div class="highlight-box my-7 overflow-y-auto overflow-x-hidden">
            ...
            <v-row
              v-for="(h, index) in data.highlights"
              :key="index"
              no-gutters
