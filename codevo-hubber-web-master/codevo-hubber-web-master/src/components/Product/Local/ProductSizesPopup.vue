...
import ProductSizesDropdown from '@/components/Product/Local/ProductSizesDropdown.vue'
...
const selectedItems = ref(data.value)
...
const onSizesSelected = (applications) => {
...
const confirmData = async () => {
  ...
  await store.commit('products/updateCurrentProduct', {
    productSizes: selectedItems.value,
  })
  onClose()
}
</script>
  ...
  <div class="hubber-dialog-box">
    <v-card-title>
      ...
      <div class="title-container d-flex flex-row justify-center">
        <div class="title">{{ $t('PRODUCT_ADD_SIZE_TITLE') }}</div>
      </div>
    </v-card-title>
