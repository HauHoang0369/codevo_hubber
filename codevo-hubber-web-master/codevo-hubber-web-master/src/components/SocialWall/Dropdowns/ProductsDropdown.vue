...
const searchProducts = async (name = '') => {
  ...
  if (dataProduct.value?.last) {
    // Last page
    return
  }

  isLoading.value = true
  const result = await new ProductService().service.callSearchedProducts(
    user.value.getOldAccessToken,
    user.value.contentLanguage.isoCode,
    user.value.market.id,
    name,
    dataProduct.value?.searchText === name ? dataProduct.value.number + 1 : 0, // Reset page if query text changed
    true
  )
  dataList.value = [...dataList.value, ...result.content]
  delete result.content
  dataProduct.value = result
  dataProduct.value.searchText = name
...
const searchInput = debounce((event) => {
  ...
  if (searchQuery.value.length > 2 || searchQuery.value.length === 0) {
    dataProduct.value.number = 0
    dataProduct.value.totalPages = 0
    dataList.value = []
...
<template>
  <div class="d-flex flex-column">
    ...
    <div class="extend-items flex-grow-1">
      ...
      >
        <div class="d-flex align-center">
          <div class="icon flex-grow-1">
            <AuthImage :height="74" :path="item.imageUrl" :width="74" />
          </div>
          <div class="info">
            <span
              :style="{ background: item.productFamily.color }"
              class="item-info-acronym"
            >
      ...
      <Observer @intersect="searchProducts(searchQuery)" />
    </div>
