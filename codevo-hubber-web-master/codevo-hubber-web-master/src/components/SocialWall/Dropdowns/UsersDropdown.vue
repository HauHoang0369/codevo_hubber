...
const onSelect = (item) => {
  emit('onSelectUser', item, tickedChecking(item))
}
...
const searchFriends = async (name = '') => {
  ...
  const result = await new SocialWallService().getFriends(data.value.number++)
...
const tickedChecking = (value) => {
  return props.selectedItems.findIndex((v) => v === value.id) !== -1
}
</script>
...
<template>
  <div class="d-flex flex-column">
    ...
    <div class="extend-items flex-grow-1">
      ...
      >
        <div class="d-flex align-center content-info">
          <div class="icon flex-grow-1">
            <AuthImage :height="38" :path="item.imageUrl" :width="38" />
          </div>
          <div class="info">
      ...
      <Observer @intersect="searchFriends(searchQuery)" />
    </div>
